...
import { CROP_LABELS, SOIL_LABELS, URGENCY_LABEL } from '../utils/labels'
...
const anyPending = computed(() => sectors.value.some(s => s.status === 'pending')) 
...
<template>
    <div class="max-w-[1180px] mx-auto px-4 md:px-8 pt-2 md:pt-6 pb-8">
      ...
      <template v-else-if="field">
        ...
        <div class="text-sm text-muted mb-5 md:mb-6">
        ...
        <!-- Pending notice -->
        <div v-if="anyPending" class="bg-blue-50 border border-blue-200 text-blue-700 text-sm px-4 py-3 rounded-2xl mb-5">
          Tenés sectores pendientes de aprobación. Un administrador los revisará para que empiecen a recibir recomendaciones.
        </div>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
          <RouterLink
            ...
            :to="s.status === 'active' ? `/sectors/${s.id}/recommendation` : `/sectors/${s.id}/edit`"
            ...
            <div class="px-4 pt-4 pb-3 flex items-start justify-between gap-2">
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-1.5 mb-1">
                  ...
                  <template v-else>
                    <span class="w-2 h-2 rounded-full bg-amber" />
                    <span class="text-[10px] font-bold uppercase tracking-wider text-amber">Pendiente</span>
                  </template>
                </div>
              ...
              <Sparkline
                v-if="s.status === 'active' && s.last_recommendation?.deficit_history?.length > 1"
            ...
            <div class="px-4 py-3 border-t border-line flex items-center justify-between bg-chip">
              <div>
                ...
                <div class="app-mono text-lg font-bold text-ink mt-0.5">
                  <template v-if="s.status !== 'active'">
                    <span class="text-sm text-muted font-normal">Por aprobar</span>
                  </template>
                  <template v-else-if="s.last_recommendation && s.last_recommendation.recommended_irrigation_mm > 0">
                    <template v-if="s.last_recommendation.time_min != null">
                      {{ formatMinutes(s.last_recommendation.time_min) }}
                    </template>
                    <template v-else-if="s.last_recommendation.volume_m3 != null">
                      {{ s.last_recommendation.volume_m3.toFixed(0) }} m³
                    </template>
                    <template v-else>
              ...
              <span class="inline-flex items-center gap-1 text-sm font-bold text-primary">
                {{ s.status === 'active' ? 'Abrir' : 'Configurar' }} <ChevronRight :size="14" />
              </span>
            </div>
          </RouterLink>
        </div>
