...
import { listPendingFields, approveField, adminUpdateSector } from '../services/admin'
...
async function confirmApproval(field) {
    approvingId.value = field.id
    approveError.value = ''
    try {
      ...
      >
        ...
        <div class="p-4">
          ...
          <p class="text-xs font-bold uppercase tracking-wide text-gray-400 mb-2">
            Sectores ({{ field.sectors?.length || 0 }})
          </p>

          <div v-if="!field.sectors || field.sectors.length === 0"
            class="text-sm text-gray-400 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 mb-3">
            Este campo no tiene sectores. No puede aprobarse.
          </div>

          <div v-else class="space-y-2 mb-4">
            <div
              v-for="s in field.sectors" :key="s.id"
              class="flex items-center justify-between gap-3 border border-gray-200 rounded-xl px-3 py-2.5"
            >
              <div class="min-w-0">
          ...
          <div v-if="approveError && approvingId === field.id" class="bg-red-50 border border-red-200 text-red-700 text-sm font-medium px-3 py-2.5 rounded-xl mb-3">
            {{ approveError }}
          </div>

          <button
            @click="confirmApproval(field)"
            :disabled="!field.sectors || field.sectors.length === 0 || !field.sectors.every(s => s.polygon_geojson) || approvingId === field.id"
            class="w-full bg-green-800 hover:bg-green-700 text-white font-bold py-2.5 rounded-xl text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {{ approvingId === field.id ? 'Aprobando...' : 'Aprobar campo' }}
          </button>
        </div>
      </div>
    </div>
